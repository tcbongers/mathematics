...
def solvePell(N, string=False):
	"""
	Look for a solution to the equation x^2 - N y^2 = 1 via generating the continued fractions.
	Assume that N is not a perfect square.
	"""
	...
	conv = 1
	while True:
		reducedFraction = cf.convergent(conv)
		num = reducedFraction.numerator
		den = reducedFraction.denominator
...
for _ in range(cap + 1):
	x, y = solvePell(_)
	if x > xmax:
		print(f'New maximum {x} at {_}')
		xmax = x
		best = _
		
print(f'Maximum x for {cap} at D = {best}')